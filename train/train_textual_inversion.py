...
def log_validation(text_encoder, tokenizer, unet, vae, args, accelerator, weight_dtype, epoch, save_path=None):
    ...
    if args.validation_prompt_file is not None:
        ...
        validation_prompts = [str(row.prompt).format(args.placeholder_token) for _, row in validation_prompts_df.iterrows()]
    ...
    if args.validation_prompt_file is not None:
            for prompt_idx, validation_prompt in enumerate(validation_prompts):
                ...
                for idx in range(args.num_validation_images):
                    ...
                    image_name = f"{epoch}_{prompt_idx}_{idx}.png".zfill(12)
                    if save_path:
                        torchvision.utils.save_image(image_save_transform(image), f"{save_path}/{image_name}")
                    images.append(image)
        ...
        for idx in range(args.num_validation_images):
            with torch.autocast("cuda"):
                ...
                if save_path:
                    ...
                    torchvision.utils.save_image(image_save_transform(image), f"{save_path}/{image_save_name}")
            images.append(image)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(text_encoder):
                ...
                progress_bar.update(1)
                global_step += 1
                logs = {"epoch": epoch, "num_update_steps_per_epoch": num_update_steps_per_epoch, "loss": loss.detach().item(), "lr": lr_scheduler.get_last_lr()[0]}
        ...
        if (args.validation_prompt is not None or args.validation_prompt_file is not None) and epoch % args.validation_epochs == 0:
            images = log_validation(
                text_encoder, tokenizer, unet, vae, args, accelerator, weight_dtype, epoch, save_path=f"{args.output_dir}"
            )
