...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        type=float,
        default=5e-4,
        help="Initial learning rate for defense parameters.",
    )
    parser.add_argument(
        "--scale_lr",
        action="store_true",
    ...
    parser.add_argument("--low_rank", default=False, action=argparse.BooleanOptionalAction, help="Whether or not to use low rank.")
    parser.add_argument(
    ...
    parser.add_argument(
        "--wandb_tmp",
        type=str,
        default="/scratch/gilbreth/zheng709/tmp",
        help=(
...
def main(args):
    ...
    for name, attn_processor in unet.attn_processors.items():
        ...
        lora_attn_processor_class = VAMMAAttnProcessor

        module = lora_attn_processor_class(hidden_size=hidden_size, cross_attention_dim=cross_attention_dim, target=target_p_name)
    ...
    for name, param in unet.named_parameters():
        if any([key in name for key in xattn_layer_names]) and 'lora' not in name and 'vamma' not in name:
            unet_attn_names.append(name)
            unet_attn_params.append(param)
            unet_attn_dict[name] = param
    ...
    for name, module in unet.attn_processors.items():
        if not args.low_rank:
            ...
            unet_lora_parameters.extend([param for name, param in module.named_parameters() if "lora" in name])
            unet_vamma_parameters.extend([param for name, param in module.named_parameters() if "vamma" in name])
    ...
    if accelerator.is_main_process:
        accelerator.init_trackers("dreambooth-lora_defense", config=vars(args))
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet):
                ...
                if args.max:
                    freeze_grad(params_to_optimize)
                    free_grad(params_to_defense)
                else:
                    if (step + epoch * len(train_dataloader)) % (args.inner_loop_steps + args.outer_loop_steps) < args.inner_loop_steps:
                        free_grad(params_to_optimize)
                        freeze_grad(params_to_defense)
                    else:
                        freeze_grad(params_to_optimize)
                        free_grad(params_to_defense)
                ...
                if args.max:
                    ...
                    if accelerator.sync_gradients:
                        accelerator.clip_grad_norm_(params_to_defense, args.max_grad_norm)
    ...
    # save defense layers
    if not args.low_rank:
        torch.save({name: param for name, param in zip(unet_attn_names, unet_attn_params)}, f"{args.output_dir}/unet_xatten_layer.pt")
    else:
        def get_attn_key(processor_key):
            key = processor_key.replace("processor.", "").replace("_vamma.up", "")
            if "to_out" in key:
                key = key.replace("to_out", "to_out.0")
            return key
        unet.requires_grad_(False)
        
        processor_state_dict = unet_attn_processors_state_dict(unet)
        for processor_key in processor_state_dict.keys():
            if "vamma" in processor_key and ".up" in processor_key:
                up_key = processor_key
                down_key = processor_key.replace(".up", ".down")
                attn_key = get_attn_key(processor_key)
                unet_attn_dict[attn_key] += processor_state_dict[up_key] @ processor_state_dict[down_key]

        torch.save(unet_attn_dict, f"{args.output_dir}/imma_unet_xatten_layer.pt")
