...
def log_validation(text_encoder, tokenizer, unet, vae, args, accelerator, weight_dtype, epoch, save_path=None):
    logger.info(
    ...
    for idx in range(args.num_validation_images):
        with torch.autocast("cuda"):
            ...
            if save_path:
                torchvision.utils.save_image(image_save_transform(image), f"{save_path}_{idx}.png")
        images.append(image)
...
def main():
    ...
    optimizer_defense = torch.optim.AdamW(
        unet_attn_params,  # optimize cross attention layers to defense
    ...
    text_encoder, optimizer, train_dataloader, lr_scheduler, optimizer_defense, lr_scheduler_defense = accelerator.prepare(
        text_encoder, optimizer, train_dataloader, lr_scheduler, optimizer_defense, lr_scheduler_defense
    )
    ...
    if accelerator.is_main_process:
        accelerator.init_trackers("textual_inversion_defense", config=vars(args))
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(text_encoder):
                ...
                if args.max:
                    freeze_grad(text_encoder.get_input_embeddings().parameters())
                    free_grad(unet_attn_params)
                else:
                    if step % (args.inner_loop_steps + args.outer_loop_steps) < args.inner_loop_steps:
                        free_grad(text_encoder.get_input_embeddings().parameters())
